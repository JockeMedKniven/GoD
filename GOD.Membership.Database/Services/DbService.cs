...
{
	private readonly GODContext _db;
	private readonly IMapper _mapper;

	public DbService(GODContext db, IMapper mapper)
	{
		_db = db;
		_mapper = mapper;
	}
	...
	public async Task<List<TDto>> GetBoolAsync<TEntity, TDto>(
	...
	//Private!!!
	private async Task<TEntity?> SingleAsync<TEntity>(Expression<Func<TEntity, bool>>
	...
	public void Include<TEntity>() where TEntity : class, IEntity
	{
		...
		{
			_db.Set<TEntity>().Include(property).Load();
		}
	}

	public async Task<List<TDto>> GetRefAsync<TEntity, TDto>()
where TEntity : class, IReferenceEntity
	...
	public bool DeleteRef<TEntity, TDto>(TDto dto)
	where TEntity : class
	where TDto : class
	{
		try
